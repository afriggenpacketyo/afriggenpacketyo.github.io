document.addEventListener('DOMContentLoaded', function() {
    ...
    // Create the CSS keyframe animations for bounce effects
    const style = document.createElement('style');
    style.textContent = `
    @keyframes bounceRight {
      0% { transform: translateX(0) scale(1); }
      40% { transform: translateX(-50px) scale(0.95); }
    ...
    // Function to apply interruptible bounce effect
    function applyInterruptibleBounce(card, direction) {
      // Immediately stop any current animation
      card.style.animation = 'none';
      
      // Force browser to recognize the animation reset
      void card.offsetWidth;
      
      // Apply the new animation immediately
      card.style.animation = direction === 'left' 
        ? 'bounceLeft 0.6s cubic-bezier(0.36, 0.07, 0.19, 0.97)' 
        : 'bounceRight 0.6s cubic-bezier(0.36, 0.07, 0.19, 0.97)';
    }

    // Add bounce effect to the navigation arrows
    if (!isMobile && navArrows.length === 2) {
      navArrows[0].addEventListener('click', () => {
        if (activeCardIndex === 0) {
          applyInterruptibleBounce(flipCards[0], 'left');
        }
      });
      
      navArrows[1].addEventListener('click', () => {
        if (activeCardIndex === flipCards.length - 1) {
          applyInterruptibleBounce(flipCards[flipCards.length - 1], 'right');
        }
      });
    }

    // Add to keyboard events - make immediately responsive
    document.addEventListener('keydown', (e) => {
      if (isMobile) return;
    ...
    // Make wheel events trigger bounce immediately without debounce
    container.addEventListener('wheel', (e) => {
      if (isMobile) return;
      
      // Immediately respond to any scroll attempt
      if (e.deltaX > 0 && activeCardIndex === flipCards.length - 1) {
        applyInterruptibleBounce(flipCards[flipCards.length - 1], 'right');
      }
      else if (e.deltaX < 0 && activeCardIndex === 0) {
        applyInterruptibleBounce(flipCards[0], 'left');
      }
    }, { passive: true });

    // Make edge detection for scrolling more responsive
    let lastScrollPosition = container.scrollLeft;
    container.addEventListener('scroll', () => {
      if (isManuallyFlipping) return;
      
      const isAtLeftEdge = activeCardIndex === 0 && container.scrollLeft <= 0;
      const isAtRightEdge = activeCardIndex === flipCards.length - 1;
      
      if ((isAtLeftEdge && lastScrollPosition !== container.scrollLeft) || 
          (isAtRightEdge && lastScrollPosition !== container.scrollLeft)) {
